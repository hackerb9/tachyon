...
char *FILENAME::createName(const char *folder, const char *file)
{
  ...
  if (folder == NULL)
  {
    result = (char *)UI_malloc(strlen(file) + 1, MALLOC043);
    if (result != NULL) strcpy(result, file);
  }
  else
  {
    result = (char *)UI_malloc(strlen(folder) + strlen(file) +1, MALLOC044);
    if (result != NULL)
    {
...
i32 FILENAME::Rename(const char *oldname, const char *newname)
{
  ...
  for (i=0; i<3; i++)
  {
    if (m_name[i] == NULL) continue;
    tw1 = m_name[i];
    tw2 = newfile.m_name[i];
    return MoveFile(tw1.Text(), tw2.Text());
  };
  return -1;
}
...
ui64 MODIFIEDTIME(i32 file)
{
  ...
  if (fn == NULL) return 0;
  tw1 = fn;
  fh = FindFirstFile(tw1.Text(), &fData);
